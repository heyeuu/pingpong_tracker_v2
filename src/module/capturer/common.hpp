...
struct Interface {
    ...
    virtual auto connected() const noexcept -> bool {
        return false;
    }
...
public:
    ...
    auto wait_image() noexcept -> ImageResult override {
        return Impl::wait_image();
    }

    static_assert(requires(Impl& impl) { impl.connect(); }, "实现必须拥有 connect 函数");
    auto connect() noexcept -> NormalResult override {
        return Impl::connect();
    }
    ...
    static_assert(requires(Impl& impl) { bool{impl.connected()}; }, "实现必须拥有 connected 函数");
